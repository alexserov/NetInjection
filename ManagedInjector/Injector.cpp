...
void Injector::Launch(System::IntPtr windowHandle, System::String^ assembly, System::String^ className, System::String^ methodName, System::String^ arguments)
{
	System::String^ assemblyClassAndMethod = assembly + "$" + className + "$" + methodName + "$" + arguments;
		...
		if (msg != NULL && msg->message == WM_GOBABYGO)
		{
			...
			if (assembly != nullptr)
			{
				...
				if (type != nullptr)
				{
					...
					{
						Object^ returnValue = methodInfo->Invoke(nullptr, gcnew array<Object^>(1) { acmSplit[3] });
